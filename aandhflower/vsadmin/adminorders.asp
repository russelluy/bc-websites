...
if (Session("loggedon") <> storesessionvalue AND request.cookies("WRITECKL")="") OR disallowlogin=TRUE then response.redirect "login.asp"
isprinter=(request.querystring("printer")="true")
if request.form("powersearch")="1" then
	if request.form("startwith")="1" then
		response.cookies("powersearch")="1"
		response.cookies("powersearch").Expires = Date()+365
	else
		response.cookies("powersearch")="0"
		response.cookies("powersearch").Expires = Date()+365
	end if
end if
%>
...
<% if NOT isprinter then response.write "<div id=""main"">" else response.write "<div id=""mainprint"">" %>
<!--#include file="inc/incorders.asp"-->
<% response.write "</div>" %>
<!-- InstanceEndEditable -->
